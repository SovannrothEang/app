...
public class TransactionRepository(AppDbContext dbContext) : ITransactionRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var (result, totalCount) = await GetPaginatedAsync(queryable, pageOption, cancellationToken);
    ...
    public async Task<(IEnumerable<Transaction> result, int totalCount)> GetPaginatedAsync(
